...
class MarkovModel:
    ...
    def get_chains(self, start_state, min_length, max_length, num_chains, include_start_state=True):
        """
        :param include_start_state: whether 'start' should be part of the sequence or not
        :param num_chains: number of sequences to generate
        :param start_state: initial state
        :param min_length: min length of chain, inclusive
        :param max_length: max length of chain, inclusive
        """
        for _ in range(num_chains):
            sequence_length = random.randrange(min_length, max_length + 1)
